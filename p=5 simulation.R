...
guji <- function(t, n, K, p, error, gamma, beta, delta_a0, delta_a1, delta_a2, delta_a3){
  ...
  for(k in 1:K){
    ...
    #X_local_IPW1 <- weight_local_IPW1 %*% X[I == k, ]; Y_local_IPW1 <- weight_local_IPW1 %*% Y[I == k]; Z_local_IPW1 <- weight_local_IPW1 %*% Z[I == k, ]
    #beta_IPW <- solve(t(X_local_IPW1) %*% X_local_IPW1) %*% t(X_local_IPW1) %*% (Y_local_IPW1 - Z_local_IPW1 %*% gamma_IPW)
    ER_IPW_beta_set <- rbind(ER_IPW_beta_set, MSE(beta_IPW, beta[k, ]))
    #cat("第", k, "组数据的IPW估计的异质参数误差为:", MSE(beta_IPW, beta[k, ]), "\n")
  }
  ...
  #weight_all_matrix <- diag(weight_all)
...
#write.csv(result, file = "9.8 (20% missing rate) p=5 results with estimated weights.csv")
#write.csv(result, file = "9.8 (40% missing rate) p=5 results with estimated weights.csv")
write.csv(result, file = "9.8 (15% missing rate) p=5 results with estimated weights.csv")
